...
## Summary files convention

Session or milestone summaries are stored in **`Cursor Summaries`** as **`[short form commit ID]_Summary.md`** (e.g. `Cursor Summaries/c39ca9d_Summary.md`). Use the current commit’s short ID from `git rev-parse --short HEAD` when creating a new summary file.
